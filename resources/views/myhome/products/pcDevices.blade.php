        ...
        <div class="card">
            ...
            <div class="card-body">
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        @foreach ($pcDevices as $item)
                            <tr>
                                <th scope="row">{{ $item->id }} </th>
                                <td>{{ $item->name }}</td>
                                <td>{{ $item->quantity }}</td>
                                <td>{{ $item->price }}</td>
                                <form action="{{ url('/admin/delete-pc-device/' . $item->id) }}" method="POST">
                                    @method('GET')
                                    @csrf
                                    <td><button class="btn btn-outline-danger text-dark" type='submit'>supp</button></td>
                                </form>
                                <form action="{{ url('/admin/edit-pc-device/' . $item->id) }}" method="POST">
                                    @method('GET')
                                    @csrf
                                    <td><button class="btn btn-outline-warning text-dark" type='submit'>mod</button></td>
                                </form>
                                <form action="{{ url('/admin/show-pc-device/' . $item->id) }}" method="POST">
                                    @method('GET')
                                    @csrf
